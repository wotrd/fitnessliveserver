...
$(function () {
    ...
    $(grid_selector).jqGrid({
        url: "/manager/collectmanager/getCollects",
        datatype: "json",
        mtype: 'POST',
        ...
        colModel: [
            ...
            {label: '类型', name: 'type', width: 200},
            {label: '价格', name: 'price', width: 200},
            {label: '用户ID', name: 'userId', width: 200},
            {label: '时间', name: 'createTime', width: 200},
    ...
    $("#deleteBusinessBtn").click(function () {
        ...
        if (rows.length > 0) {
            $.messager.confirm("温馨提示", "是否确定删除所选记录？", function () {
                $.ajax({
                    url: "/manager/collectmanager/delete",
                    cache: false,
                    type: "post",
                    data: {"ids": uids.join(",")},
                    beforeSend: function () {
