...
$(function () {
    ...
    $(grid_selector).jqGrid({
        url: "/manager/infomanager/info/list",
        datatype: "json",
        mtype: 'POST',
    ...
    $("#deleteBusinessBtn").click(function () {
        ...
        if (rows.length > 0) {
            $.messager.confirm("温馨提示", "是否确定删除所选记录？", function () {
                $.ajax({
                    url: "/manager/infomanager/delete",
                    cache: false,
                    type: "post",
                    data: {"ids": uids.join(",")},
                    beforeSend: function () {
...
function initUpdateData() {
    ...
    $('#upsummary').val(data.summary);
    $('#upeditor').val(data.content);
...
function saveUpdateBusiness() {
    var submit_option = {
        url: "/manager/infomanager/update",//默认是form action
        method: "POST",
        success: function (result) {
            if (result.flag == true) {
                $.messager.alert('温馨提示', result.message);
